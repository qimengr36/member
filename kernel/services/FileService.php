...
class FileService
{
     ...
     * @return boolean
     */
    public function removeDir($dir_path, bool $is_all = false)
    {
    ...
    public static function listInfo(string $file)
    {
        ...
        $dir['pathname'] = strstr(php_uname('s'), 'Windows') ? str_replace('\\', '\\\\', realpath($file)) : realpath(
            $file
        );//返回绝对路径名。
    ...
    public function getUploadFileInfo($file)
    {
        ...
        $info['type'] = strtolower(
            trim(stripslashes(preg_replace("/^(.+?);.*$/", "\\1", $file_info['type'])), '"')
        );//取得文件类型
...
//        '售价', '单价', '建议零售价',
//        '库存限制', '库存', '重量', '体积', '编号',
//        '起购数量(状态)', '起购数量', '详情', '简介', '启用日期', '截止日期', '排序',
//    ];
    public array $goodsHeader = [
        'code',//コード 商品编号
        'goods_name',//商品名  商品名称
        'goods_name_kana',//商品名（カナ） 商品假名
        'cate_name',//カテゴリー  商品分类
        'manu_name',//生産者  厂商
        'manu_bar_code',//生産者商品コード  厂商商品编号
        'tax',//税率%
        'unit_name',//単位  商品单位
        'sell_unit_name',//発注単位  销售单位
        'item_name',//仕様  规格值
        'slider_image',//画像  商品图片
        'is_show',//公開  上下架
        'shelf_life',//賞味期限  保质期
        'spec_type',//複数規格(ON/OFF)  多规格（状态）
        'spec_temp_name',//規格テンプレート名  规格模板名称
        'sku_type_value',//規格＋全部規格値  规格类型值
        'sku_value',//規格値  规格值组合
        'price',//価格  售价
        'unit_price',//原単価  单价
        'retail_price',//希望売価  建议零售价
        'is_stock',//在庫制限  库存限制
        'stock',//在庫数  库存
        'volume',//重さ(kg)  重量
        'weight',//大きさ(m³)  体积
        'bar_code',//JAN  编号
        'is_buy_minimum',//最低注文数(ON/OFF)  起购数量(状态)
        'buy_minimum',//最低注文数  起购数量
        'description',//商品詳細  详情
        'goods_info',//商品概要  简介
        'sale_start_time',//利用開始日  启用日期
        'sale_end_time',//利用終了日  截止日期
        'sort',//表示順位  排序
    ];
    public array $goodsPriceHeader = [
        'code',//コード code
        'bar_code',//JAN
        'goods_name',//商品名
        'cate_name',//カテゴリー  商品分类
        ...
        'goods_unit_price',//原単価  单价
        'goods_price',//価格  售价
        'goods_retail_price',//希望売価  建议零售价
        'tag_id',//模板ID
        'tag_name',//模板名称
        'unit_price',//卸単価
        'price',//卸価格
    ...
    public function readHeaderExcel($filePath, $type, int $row_num = 1)
    {
        if (!$filePath) {
            return false;
        }
        $pathInfo = pathinfo($filePath, PATHINFO_EXTENSION);
        if (!$pathInfo || ($pathInfo != "xlsx" && $pathInfo != "xls")) {
        ...
        try {
            ...
            $data = [];
            if ($type == 'goods') {
                $verify = $this->goodsHeader;
                for ($j = 1; $j <= count($verify); $j++) {
                    $data[] = $this->objToStr(
                        $sheet->getCell(Coordinate::stringFromColumnIndex($j).$row_num)->getValue()
                    );
                }
            }
            return $data;
     ...
     */
    public function readExcel($filePath, $type, int $row_num = 1)
    {
        if (!$filePath) {
            return false;
        }
        ...
        try {
            ...
            $title = [];
            for ($j = 0; $j < count($verify); $j++) {
                $k = $j + 1;
                $title[$j] = $this->objToStr($sheet->getCell(Coordinate::stringFromColumnIndex($k). 1)->getValue());
            }
                        ...
                        if ($i != 1) {
                            switch ($verify[$j]) {
                                ...
                                case 'sale_start_time':
                                    $value = $this->objToStr(
                                        $sheet->getCell(Coordinate::stringFromColumnIndex($k).$i)->getValue()
                                    );
                                    if ($value) {
                                        try {
                                            ...
                                            $array[$verify[$j]] = $dateValue->format('Y-m-d');
                                        } catch (\Exception $e) {
                                            $val = $this->objToStr(
                                                $sheet->getCell(Coordinate::stringFromColumnIndex($k).$i)->getValue()
                                            );
                                            $array[$verify[$j]] = date('Y-m-d', strtotime($val));
                                        }
                                    } else {
                                        $array[$verify[$j]] = '';
                                    }
                                ...
                                default:
                                    ...
                                    $array[$verify[$j]] = trim($value);
                            }
                        }
                    }

                    if ($i != 1) {
                        if (array_filter(array_values($array))) {
                            [
                                'slider_image'   => $slider_image,//商品图片
                                'is_show'        => $is_show,//上下架 yes no
                                'spec_type'      => $spec_type,//规格类型 yes多规格 no单规格
                                'price'          => $price,//售价
                                'is_stock'       => $is_stock,//库存限制 yes no
                                'is_buy_minimum' => $is_buy_minimum,//起购数量(状态) yes no
                                'sort'           => $sort,//排序
                            ] = $array;

                            if (!$is_show) {
                                throw new AdminException('400358', ['name' => $title[9]]);
                            }
                            if (!$spec_type) {
                                throw new AdminException('400358', ['name' => $title[11]]);
                            }
                            if (!$is_stock) {
                                throw new AdminException('400358', ['name' => $title[16]]);
                            }
                            //is_show 状态（0：未上架，1：上架）
                            if (strtolower($is_show) == 'on') {
                                $is_show = 1;
                            } else {
                                $is_show = 0;
                            }
                            //is_buy_minimum 起购数 1开启0关闭
                            if (strtolower($is_buy_minimum) == 'on') {
                                $is_buy_minimum = 1;
                            } else {
                                $is_buy_minimum = 0;
                            }
                            //spec_type 规格 0单 1多
                            if (strtolower($spec_type) == 'on') {
                                $spec_type = 1;
                            } else {
                                $spec_type = 0;
                            }
                            //is_stock 是否开启库存  1.On 2.Off
                            if (strtolower($is_stock) == 'on') {
                                $is_stock = 1;
                            } else {
                                $is_stock = 2;
                            }
